...
void Java_com_jackiehou_mp3lame_VoiceConvert_initEncoder(JNIEnv *env,
		jobject jobj, jint in_num_channels, jint in_samplerate, jint in_brate,
		jint in_mode, jint in_quality,jint jendian) {
	lame = lame_init();
...
void Java_com_jackiehou_mp3lame_VoiceConvert_destroyEncoder(
		JNIEnv *env, jobject jobj) {
	int res = lame_close(lame);
	LOGD("Deinit returned: %d", res);
}

void Java_com_jackiehou_mp3lame_VoiceConvert_encodeFile(JNIEnv *env,
